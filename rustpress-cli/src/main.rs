...
use rustpress_core::{
    ...
    render_slideshow_vue, render_with_template, rss::{parse_date_to_rfc2822, strip_html, RssFeed, RssItem},
...
fn render_nav_tree(chapters: &[Chapter], _base_path: &Path) -> String {
    ...
    for chapter in chapters {
        ...
        let href = if path_str.starts_with("src/") {
            format!("{}.html", &path_str[4..])
        } else {
...
fn build_chapter_store(
    ...
    current_idx: usize,
    content: String,
    all_items: &[(std::path::PathBuf, std::path::PathBuf, String, String, std::collections::HashMap<String, String>)],
) -> ChapterStore {
    let url = chapter
        .path
        .to_string_lossy()
    ...
    let prev_chapter = if current_idx > 0 {
        ...
        Some(ChapterNav {
            ...
            url: prev.path.to_string_lossy().replace(".md", "").replace("src/", "") + ".html",
        })
    } else {
        None
    };
    ...
    let next_chapter = if current_idx < all_chapters.len() - 1 {
        ...
        Some(ChapterNav {
            ...
            url: next.path.to_string_lossy().replace(".md", "").replace("src/", "") + ".html",
        })
    } else {
        None
    };
    ...
    let children: Vec<ChapterStore> = chapter
        ...
        .enumerate()
        .map(|(i, child)| {
            let child_idx = all_chapters.iter().position(|c| c.path == child.path).unwrap_or(current_idx);
            ...
            let child_content = all_items
                ...
                .find(|(rp, _, _, _, _)| {
                    rp.to_string_lossy()
                        .replace(".md", "")
                        .replace("src/", "")
                        == child.path.to_string_lossy().replace(".md", "").replace("src/", "")
                })
...
fn build_all_chapter_stores(
    ...
    let all_refs: Vec<&Chapter> = all.iter().map(|c| *c).collect();
            ...
            let content = all_items
                ...
                .find(|(rp, _, _, _, _)| {
                    rp.to_string_lossy()
                        .replace(".md", "")
                        .replace("src/", "")
                        == ch.path.to_string_lossy().replace(".md", "").replace("src/", "")
                })
                .map(|(_, _, _, rendered, _)| rendered.clone())
                .unwrap_or_default();
...
fn render_blog_post(item: &ContentItem, posts: &[(&str, &str)]) -> String {
    let template =
        get_builtin_template("blog/post").unwrap_or(get_builtin_template("default").unwrap());

    let prev_next_html = if !posts.is_empty() {
        let mut html = String::new();
        for (i, (slug, post_title)) in posts.iter().enumerate() {
            if i > 0 {
    ...
    // Add prev/next to metadata for template rendering
    let mut metadata = item.metadata.clone();
    metadata.insert("prev_post".to_string(), prev_next_html);
    metadata.insert("next_post".to_string(), String::new());

    let content_item = ContentItem {
        path: item.path.clone(),
        content: item.content.clone(),
        metadata,
        rendered_content: item.rendered_content.clone(),
        related_items: item.related_items.clone(),
        image_references: item.image_references.clone(),
    };

    render_with_template(&content_item, "blog/post", template)
}
...
fn main() -> Result<()> {
    ...
    match &cli.command {
        Commands::Convert {
            ...
            let item =
                parse_markdown_with_path(&content, Some(&registry), Some(input.clone().into()));

            let html = if template.as_ref().map_or(false, |p| {
                p.to_string_lossy().contains("slideshow")
            }) {
            ...
            for (rel_path, output_path, file_stem, rendered, metadata) in &all_items {
                ...
                let html = if let Some(ref tmpl_name) = builtin_template_name {
                    ...
                    if is_landing {
                        ...
                        let current_store = all_chapters_flat.iter()
                            .find(|ch| ch.url.contains(&rel_path_str) || rel_path_str.ends_with(&ch.url.replace(".html", "")))
                            .cloned();
                        if let Some(current) = current_store {
                            render_mdbook_vue(&book_title, &chapter_stores, &current, &search_index)
                        } else {
                            render_with_template(&ContentItem {
                                path: None,
                                content: String::new(),
                ...
                if html.len() < 500 {
                    eprintln!("WARNING: Generated file '{}' is very small ({} bytes). Possible template issue.", output_path.display(), html.len());
                }
...
<body>
    <p>Redirecting to <a href="{}.html">{}</a>...</p>
                ...
                .as_ref()
                .map_or(false, |t| t == "blog" || t == "blog/index")
            {
