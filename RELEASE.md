...
Step-by-step guide for cutting a new release of `rustpress`. This procedure is
...
- The publishing crate is `rustpress` (the CLI in `rustpress/`).
- The publishing target is crates.io.
- The mirror target is `github.com/hahihula/rustpress`.
- The Docker registry is Docker Hub (`hahihula/rustpress` and
  `hahihula/rustpress-alpine`).
...
| `GITHUB_TOKEN`    | https://github.com/settings/tokens — fine-grained token for the `hahihula/rustpress` repo, with `Contents: Read & Write` (for the mirror push) |
...
cargo build --release --package rustpress
```
...
for breaking changes that affect users of the CLI or the `rustpress-core` API
...
The only crate that is published to crates.io is `rustpress` (the CLI). Update
its version in `rustpress/Cargo.toml`:
...
If `rustpress-core` or `rustpress-wasm` changed in a way that affects their
...
| `build:linux:cli`       | build            | Builds release binary, uploads `rustpress-linux-x86_64` artifact |
| `mirror:github`         | mirror           | Mirrors the tag/branch to `github.com/hahihula/rustpress` |
...
2. **`docker:build:using-template`** — builds and pushes the canonical image
   to Docker Hub as `hahihula/rustpress` (tags: `$CI_COMMIT_TAG`, `latest`).
   This is the image most users will pull. **Run this first.**
3. **`docker:build:alpine`** — builds and pushes the same image to Docker
   Hub as `hahihula/rustpress-alpine` (tags: `$CI_COMMIT_TAG`, `latest`).
4. **`docker:build`** — builds and pushes an additional image to Docker
   Hub as `hahihula/rustpress` (no `latest`/`tag` split by default; pushes
...
cargo install rustpress --version 0.2.0
rustpress --version

# GitHub mirror (the tag should be visible)
git ls-remote --tags https://github.com/hahihula/rustpress.git | grep v0.2.0

# GitHub release page
# https://github.com/hahihula/rustpress/releases/tag/v0.2.0

# GitLab release page
# https://gitlab.com/hahihula/rustpress/-/releases/v0.2.0

# Docker images
docker pull hahihula/rustpress:0.2.0
docker pull hahihula/rustpress-alpine:0.2.0
```
...
#   2. play docker:build:using-template  (canonical: hahihula/rustpress)
#   3. play docker:build                 (also hahihula/rustpress)
#   4. play docker:build:alpine          (hahihula/rustpress-alpine)
```
